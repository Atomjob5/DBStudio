...
方言切分器会把仅由空白和完整注释组成的片段视为不可执行内容：它们不会送入 Druid，也不会参与脚本执行。SQL 前置或内联注释会原样保留；MySQL `/*! ... */` 版本注释按可执行内容处理。“执行当前语句”先检查光标所在物理行：纯注释行和空白行返回既有 `EMPTY_SQL`，同行同时存在 SQL 和注释时执行该行所属的完整语句；不会跨行挑选临近 SQL。同一行存在多条语句且光标位于间隔或尾注释时，优先选择左侧语句。未闭合块注释仍由本地结构诊断报告 `SQL_UNCLOSED_COMMENT`。
