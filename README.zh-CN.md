...
- 执行按钮下拉菜单提供“查看执行计划”：MySQL、Oracle、OceanBase Oracle 的估算计划在独立结果标签中展示，支持树形表格、节点详情、原始文本和复制
...
选择 SQL 后使用 **执行 → 查看执行计划**；没有选区时分析光标所在语句。每次处理一条查询或数据库支持的 INSERT、UPDATE、DELETE、MERGE，只获取估算计划，不运行目标语句。MySQL 不支持 MERGE。

计划标签追加在现有结果之后，支持关闭；普通执行仍替换当前编辑器的全部结果。计划标签提供树形/文本切换、展开/折叠和算子详情，不提供数据编辑、分页或自动刷新。成本和行数都是数据库返回的估算，缺失值显示“—”；扫描和连接方式标记不是性能故障判定。

Oracle 需要现有 PLAN_TABLE 和 DBMS_XPLAN 的访问权限。应用不创建计划表，并通过局部回滚清理本次计划记录，保留已有事务。OceanBase 使用自身 EXPLAIN EXTENDED 格式；无法解析的版本仍提供原始文本。快捷键可在设置中绑定，默认不绑定。

已验证范围及真实数据库待验项目见[执行计划验证记录](docs/execution-plan-validation.md)。
